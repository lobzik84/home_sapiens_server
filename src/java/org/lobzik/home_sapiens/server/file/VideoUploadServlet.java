 ...
 * @author lobzik
 */
@WebServlet(name = "VideoUploadServlet", urlPatterns = {"/video/*"})
        ...
        if (request.getMethod().equalsIgnoreCase("post")) {
            ...
            if (pos ==0) {
                ...
                try (Connection conn = DBTools.openConnection(CommonData.dataSourceName)) {
                    int  videoFileId = DBTools.insertRow("video_files", dbMap, conn);
                    boxLog.info("Begin video downloading from box, saving to file id=" + videoFileId);
                }
